...
class ScienceFileTable(Base.Base):  # type: ignore
    ...
    science_product_id = Column(
        Integer,
        ForeignKey(f"{CONFIGURATION.mission_name}_science_product.science_product_id"),
    )

    # File Type Of Science File (Foreign Key)
    file_type = Column(
        String, ForeignKey(f"{CONFIGURATION.mission_name}_file_type.short_name")
    )

    # File Level Of Science File (Foreign Key)
    file_level = Column(
        String, ForeignKey(f"{CONFIGURATION.mission_name}_file_level.short_name")
    )
