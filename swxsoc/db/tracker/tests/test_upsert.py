...
def test_upsert_inserts_new_file_type() -> None:
    ...
    extended_types = get_config().file_types + [new_type]
...
def test_upsert_updates_existing_file_type() -> None:
    ...
    mutated_types = [dict(ft) for ft in get_config().file_types]
...
def test_upsert_inserts_new_instrument() -> None:
    ...
    max_id = max(inst["instrument_id"] for inst in get_config().instruments)
    new_instrument = {
    ...
    extended = get_config().instruments + [new_instrument]
...
def test_upsert_updates_existing_instrument() -> None:
    ...
    mutated = [dict(inst) for inst in get_config().instruments]
...
def test_upsert_updates_existing_instrument_configuration() -> None:
    ...
    mutated = [dict(ic) for ic in get_config().instrument_configurations]
...
def test_existing_fk_references_survive_upsert() -> None:
    ...
    first_config_id = get_config().instrument_configurations[0]["instrument_configuration_id"]
    with session.begin() as s:
