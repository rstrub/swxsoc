...
_cached_association: Any = None
_cached_for_mission: str | None = None


def return_class() -> Any:
    """Return the StatusTable class, creating it if necessary."""
    global _cached_class, _cached_association, _cached_for_mission
    ...
    if _cached_class is None or _cached_for_mission != current_mission:
        ...
        _cached_association = Table(
            f"{current_mission}_status_origin_association",
            mission_base.metadata,
            Column("status_id", Integer, ForeignKey(f"{current_mission}_status.status_id"), primary_key=True),
            Column(
                "origin_file_id",
                Integer,
                ForeignKey(f"{current_mission}_science_file.science_file_id"),
                primary_key=True,
            ),
        )
        
        class StatusTable(mission_base):  # type: ignore
            __tablename__ = f"{current_mission}_status"
            ...
            science_file_id = Column(
                Integer, ForeignKey(f"{current_mission}_science_file.science_file_id"), nullable=False
            )

            # Many-to-many relationship to origin files
            origin_files = relationship(
                "ScienceFileTable",
                secondary=_cached_association,
